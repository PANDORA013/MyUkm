...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        chatForm.addEventListener('submit', function(e) {
            ...
            if (message) {
                ...
                .catch(error => {
                    ...
                    // Jangan tampilkan error mute sebagai alert merah (hanya alert kuning dari showMutedAlert)
                    if (error.message !== 'Session expired' && !(error.message && error.message.includes('di-mute'))) {
        ...
        async function loadLatestMessages() {
            try {
                await retryWithTokenRefresh(async () => {
                    const response = await fetch(`{{ route('chat.messages') }}?group_id=${groupId}&after=${lastMessageId}&limit=10`, {
                        headers: {
                            'X-CSRF-TOKEN': csrfToken,
                            'X-Requested-With': 'XMLHttpRequest'
                        }
        ...
        async function loadOnlineMembers() {
            ...
            try {
                await retryWithTokenRefresh(async () => {
                    const response = await fetch(`{{ route('chat.online-members') }}?group_id=${groupId}`, {
                        headers: {
                            'X-CSRF-TOKEN': csrfToken,
                            'X-Requested-With': 'XMLHttpRequest'
                        }
        ...
        async function updateOnlineStatus() {
            ...
            try {
                await retryWithTokenRefresh(async () => {
                    const response = await fetch('{{ route('chat.update-online-status') }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken,
