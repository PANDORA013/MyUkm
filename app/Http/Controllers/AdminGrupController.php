...
class AdminGrupController extends Controller
{
    ...
    public function lihatAnggota(Request $request)
    {
        ...
        if ($selectedGroupId) {
            ...
            $groupDetails = [
                ...
                'ex_members' => $result['ex_members'] ?? collect()
            ];
        } else {
            $groupDetails = [
                'selectedGroup' => null,
    ...
    public function updateDescription(Request $request)
    {
        ...
        if (!$ukmId) {
            return back()->with('error', 'ID UKM tidak ditemukan');
        }
        $result = $this->groupAdminService->updateUkmDescription($user, $ukmId, $description);
        if ($result['success']) {
            return back()->with('success', $result['message']);
        } else {
            return back()->with('error', $result['error']);
    ...
    /**
     * Display ex-members (history) page
     */
    public function riwayatAnggota(Request $request)
    {
        ...
        return view('grup.riwayat_anggota', array_merge($dashboardData, $groupDetails));
    }
}
