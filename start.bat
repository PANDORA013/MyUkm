...
echo [Q] Quit
echo.
REM --- Check for invalid APP_KEY and auto-fix if needed ---
for /f "usebackq tokens=*" %%A in ('.env') do (
    echo %%A | findstr /B "APP_KEY=" >nul && set "APPKEYLINE=%%A"
)
if defined APPKEYLINE (
    setlocal EnableDelayedExpansion
    set "KEYVAL=!APPKEYLINE:~8!"
    REM Check if key is empty or too short (less than 32 chars for base64)
    if "!KEYVAL!"=="" goto FIX_APP_KEY
    set "KEYLEN=!KEYVAL!"
    set "KEYLEN=!KEYLEN:~0,7!"
    if "!KEYLEN!"=="base64:" (
        set "KEYVAL=!KEYVAL:~7!"
    )
    set "LEN=0"
    for %%C in (!KEYVAL!) do set /a LEN+=1
    if !LEN! lss 32 goto FIX_APP_KEY
    endlocal
)
...
:FIX_APP_KEY
echo Detected invalid or missing APP_KEY. Regenerating...
php artisan key:generate
echo ✓ APP_KEY regenerated!
